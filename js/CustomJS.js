function Sunset(...buildings) {
    ...
    for (let loop = 1; loop < buildings.length; loop++) {
        // assume no building is bigger
        let bigger = false;
        // Check if any building before it is larger
        for (let loop2 = 0; loop2 < loop; loop2++) { 
            // previous greater than current?
            if (buildings[loop2] >= buildings[loop]) { 
    ...
    results.push(see);
    document.getElementById("seeResult").innerText = results;
    // How many are blind?
    let blind = results.filter(x => x == 'Blind').length;
    results.push(blind);
    document.getElementById("notSeeResult").innerText = results;
